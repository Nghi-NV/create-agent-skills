...
// Path to bundled skills and workflows
...
/**
 * Parse workflow frontmatter to extract metadata
 */
...
/**
 * Get all available workflows from the bundled workflows directory
 */
...
/**
 * Get workflow install path (always workspace-level)
 */
function getWorkflowInstallPath() {
  return path.join(process.cwd(), '.agent', 'workflows');
}

/**
 * Install selected workflows to the target location
 */
async function installWorkflows(workflows) {
  const targetDir = getWorkflowInstallPath();
  const results = [];

  await fs.ensureDir(targetDir);

  for (const workflow of workflows) {
    const targetPath = path.join(targetDir, `${workflow.name}.md`);
